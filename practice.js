...
//Code here
favoriteThings.car = '1936 bentley'
favoriteThings.brand = 'bentley'
/*
...
for (const value in deleteTheBigNumbers) {
  ...
  delete deleteTheBigNumbers[value] > 100 
  }
}
/*
...
//Code Here
function usCanadaBorder(
  arr = ['latitude', 'longitude']
  ) 
  {
  const [ latitude, longitude ] = arr
  if (latitude > -123, latitude < -95) {
    return 'true'
} else
    return 'false'
  }
...
for (let i = 0)
  ...
  if (
    employees.firstName = 'Theo'
    ) 
    {
    delete employees[2]
  }
  if (
    employees.firstName = 'Lorie'
    ) 
  {
